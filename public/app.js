...
function metricCard(label, value, tone = 'neutral') {
  return `<article class="metric tone-${tone}"><span>${label}</span><strong>${value}</strong></article>`;
}
...
function statusTone(status) {
  if (['healthy', 'published', 'improved', 'completed'].includes(status)) return 'good';
  if (['lagging', 'triage', 'generating_eval', 'draft', 'watch', 'reviewing'].includes(status)) return 'warn';
  if (['critical', 'regressed', 'failed'].includes(status)) return 'danger';
  return 'neutral';
}
...
async function loadDashboard() {
  ...
  document.getElementById('sourceCards').innerHTML = data.sources.map((source) => `
    <article class="info-card">
      <div class="card-head"><h3>${source.name}</h3>${badge(source.status, statusTone(source.status))}</div>
      <p>${source.kind}</p>
      <p class="muted">Owner: ${source.owner}</p>
      <div class="info-stats"><strong>${source.records_24h}</strong><span>records / 24h</span><strong>${source.freshness_minutes}m</strong><span>freshness lag</span></div>
  ...
  document.getElementById('ingestList').innerHTML = data.recentIngests.map((batch) => `
    <article class="stack-card">
      <div class="card-head"><h3>${batch.source_name}</h3>${badge(batch.status, statusTone(batch.status))}</div>
      <p>${batch.kind}</p>
  ...
  document.getElementById('clusterActivityList').innerHTML = data.recentClusterActivity.map((run) => `
    <article class="stack-card">
      <div class="card-head"><h3>${run.title}</h3>${badge(run.status, statusTone(run.status))}</div>
      <p>${run.strategy}</p>
      <p class="muted">${run.trace_count} traces · owner ${run.owner}</p>
  ...
  document.getElementById('clusterCards').innerHTML = data.clusters.map((cluster) => `
    <article class="cluster-card">
      <div class="card-head"><h3>${cluster.title}</h3>${badge(cluster.severity, severityTone(cluster.severity))}</div>
      <p>${badge(cluster.status, statusTone(cluster.status))} ${badge(cluster.root_cause_label, 'neutral')}</p>
      <p class="muted">${cluster.root_cause_hypothesis}</p>
      <div class="info-stats compact"><strong>${cluster.trace_count}</strong><span>source traces</span><strong>${cluster.recompute_count}</strong><span>recomputes</span></div>
      <p class="muted">Owner: ${cluster.owner} · ${cluster.triage_note || 'No triage note'}</p>
      <p class="muted">Updated ${formatTimestamp(cluster.last_seen_at)}</p>
    </article>
  `).join('');
  ...
  document.getElementById('evalCasesTable').innerHTML = data.evalCases.map((item) => `
    <tr>
      ...
      <td>${item.cluster_title}<div class="muted">${badge(item.cluster_severity, severityTone(item.cluster_severity))}</div></td>
  ...
  document.getElementById('replayList').innerHTML = data.replayRuns.map((run) => `
    <article class="stack-card">
      <div class="card-head"><h3>${run.eval_case_name}</h3>${badge(run.verdict, statusTone(run.verdict))}</div>
  ...
  document.getElementById('exportList').innerHTML = data.exports.map((batch) => `
    <article class="stack-card">
      <div class="card-head"><h3>${batch.target_path}</h3>${badge(batch.status, statusTone(batch.status))}</div>
